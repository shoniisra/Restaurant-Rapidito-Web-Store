...
class DefaultController extends BaseController
{
    ...
    public function actionContactUs()
    {
        ...
        {
            $message = UsniAdaptor::t('applicationflash', 'GRACIAS POR ESCRIBIRNOS, LE CONTACTAREMOS PRONTO');
            FlashUtil::setMessage('success', $message);
            $this->refresh();
        }
        elseif($formDTO->getIsTransactionSuccess() === false)
        {
            FlashUtil::setMessage('error', UsniAdaptor::t('applicationflash', 'HUBO UN ERROR AL ENVIAR EL MENSAJE'));
            return $this->refresh();
        }
        else
        {
